...
class WebCallReturn:
	...
	def _httpResponse(self):
		...
		'''
		# from model.django.desert import settings
	...
	def httpResponse(self):
		return HttpResponse(self._httpResponse(),content_type='application/json')
...
def FailCallReturn(_ecode=errors.ErrorDefs.InternalException,_emsg=''):
	ecode = _ecode
	emsg = _emsg
	if isinstance(_ecode,tuple):
		ecode,emsg = _ecode
	if isinstance(_ecode,errors.ErrorDefs.InnerError):
		ecode,emsg = _ecode.code,_ecode.msg
	if _emsg:
		emsg = _emsg
	if not _emsg and _ecode is errors.ErrorDefs.InternalException:
